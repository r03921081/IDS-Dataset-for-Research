...
train_data = raw_train_data.drop(['id', 'attack_cat', 'label'], axis=1)
train_target = raw_train_data['label']
i = 0
for i in range(82332):
	if train_data["proto"][i] in proto_table:
		train_data["proto"][i] = proto_table[train_data["proto"][i]]
	else:
		train_data["proto"][i] = 0
	if train_data["service"][i] in service_table:
		train_data["service"][i] = service_table[train_data["service"][i]]
	else:
		train_data["service"][i] = 0
	if train_data["state"][i] in state_table:
		train_data["state"][i] = state_table[train_data["state"][i]]
	else:
		train_data["state"][i] = 0

test_data = raw_test_data.drop(['id', 'attack_cat', 'label'], axis=1)
test_target = raw_test_data['label']
i = 0
for i in range(175341):
	if test_data["proto"][i] in proto_table:
		test_data["proto"][i] = proto_table[test_data["proto"][i]]
	else:
		test_data["proto"][i] = 0
	if test_data["service"][i] in service_table:
		test_data["service"][i] = service_table[test_data["service"][i]]
	else:
		test_data["service"][i] = 0
	if test_data["state"][i] in state_table:
		test_data["state"][i] = state_table[test_data["state"][i]]
	else:
		test_data["state"][i] = 0

np_train_data = train_data.as_matrix()
np_train_target = train_target.as_matrix()
np_test_data = test_data.as_matrix()
np_test_target = test_target.as_matrix()

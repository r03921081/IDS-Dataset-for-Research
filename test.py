...
train_data = raw_train_data.drop(['id','proto', 'state', 'service', 'attack_cat', 'label'], axis=1)

discretization = pd.DataFrame(index = train_data.index, columns = train_data.columns)
#print(discretization)

for feature in train_data.columns:
	temp = train_data[feature].as_matrix()
	discretization[feature] = pd.cut(temp, 10)
